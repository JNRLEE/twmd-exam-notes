...
| 情境/藥物                                 | 方向                                                                                                         |
| ------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| Mechanical valve                      | Tx＝warfarin；C/I＝DOAC。                                                                                      |
...
| Aspirin                               | Ind＝coronary artery disease（CAD）/ACS/PCI 等動脈血栓的抗血小板；陷阱＝不可替代 atrial fibrillation（AF）的 anticoagulation（抗凝血）。 |

### 糖尿病藥：心腎 benefit vs 水腫風險

- SGLT2 inhibitors：糖尿病藥，MOA＝抑制近端小管 glucose reabsorption → glucosuria；Ind＝type 2 diabetes mellitus（T2DM），也有 heart failure with reduced ejection fraction（HFrEF）/chronic kidney disease（CKD） benefit；SE＝genital infection、euglycemic DKA。
- Pioglitazone（TZD）：糖尿病藥，MOA＝PPAR-gamma agonist，提高 insulin sensitivity；SE＝fluid retention/weight gain；C/I＝congestive heart failure（CHF）或明顯水腫。
...
- Asthma＋hypertension（HTN）：C/I＝nonselective beta-blocker（bronchospasm 風險）。
- Esmolol：超短效 beta-blocker，紅血球 esterase 代謝；適合需要快速開關的 rate/BP 控制。
...
- Syndrome of inappropriate antidiuretic hormone secretion（SIADH）：限水。
- Central diabetes insipidus（DI）：DDAVP 有效。
- Nephrogenic DI：DDAVP 無效，處理腎性原因。
...
| Migraine                                     | 急性 Tx＝triptan/NSAID；預防 Tx＝beta-blocker/topiramate/TCA；慢性 migraine 可用 onabotulinumtoxinA（Botox，非急性、非 episodic）；C/I＝triptan（CAD；血管收縮風險）。 |
